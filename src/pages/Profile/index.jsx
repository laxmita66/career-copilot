...
import {
  ...
  resumeHistory, analysisHistory, defaultSettings, securityInfo,
...
const inputCls = 'w-full bg-gray-800 border border-gray-700 rounded-xl px-3.5 py-2.5 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed'
...
const Profile = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 grid sm:grid-cols-2 gap-4 content-start">
          {placementStats.map((s) => (
            <div key={s.label}
              className="bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-gray-700 hover:-translate-y-0.5 transition-all duration-200">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${s.up ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400'}`}>
                  {s.up ? <RiArrowUpLine size={11} /> : <RiArrowDownLine size={11} />}
                  {s.trend}
                </span>
              </div>
              {/* Sparkline for ATS */}
      ...
      {/* ── Row 2: Resume History + Analysis History ───── */}
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <SectionTitle
            ...
            subtitle="All uploaded resume versions"
          />
            ...
            {resumeHistory.map((r) => (
              <div key={r.id} className="flex items-center gap-3 bg-gray-800/50 border border-gray-700/50 hover:border-gray-600 rounded-xl px-4 py-3 transition-colors group">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-gray-600 hover:text-red-400 hover:bg-red-500/10 transition-all">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <SectionTitle
            ...
            subtitle="Recent AI analyses performed"
          />
      ...
      {/* ── Row 3: Account Settings + Security ────────── */}
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          ...
          <div className="divide-y divide-gray-800">
            <Toggle
              ...
              onChange={(v) => toggleSetting('emailNotifications')}
              label="Email Notifications"
              sub="Receive updates and analysis results via email"
            />
            <Toggle
              checked={settings.pushNotifications}
              onChange={(v) => toggleSetting('pushNotifications')}
              label="Push Notifications"
              sub="Browser push notifications for important alerts"
            />
            <Toggle
              checked={settings.weeklyReport}
              onChange={(v) => toggleSetting('weeklyReport')}
              label="Weekly Progress Report"
              sub="Receive a weekly summary of your career progress"
            />
            <Toggle
              checked={settings.darkMode}
              onChange={(v) => { toggleSetting('darkMode'); showToastMsg('Theme preference saved!') }}
              label={<span className="flex items-center gap-1.5"><RiMoonLine size={13} />Dark Mode</span>}
              sub="UI only — no functional effect in this demo"
            />
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          <SectionTitle
            ...
            title="Security"
            subtitle="Account security and session info"
          />
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between py-3 border-b border-gray-800 last:border-0">
                ...
                <span className={`text-xs font-semibold ${
                  value === 'Active' || value === 'Enabled' ? 'text-green-400' :
                  value === 'Disabled' ? 'text-orange-400' : 'text-gray-300'
                }`}>{value}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Button variant="secondary" size="sm" onClick={() => showToastMsg('2FA setup coming soon!')}>
              <RiShieldCheckLine size={14} />
              Enable 2FA
            </Button>
            <Button variant="danger" size="sm" onClick={() => showToastMsg('Logged out successfully!')}>
